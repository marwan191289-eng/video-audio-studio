...
import {
  ArrowRight, Mic, Image as ImageIcon, User, Palette, Wand2,
  Upload, Download, Loader2, CheckCircle2, AlertCircle, X,
  ChevronDown, ChevronUp, Trash2, Plus, Info,
} from "lucide-react";
...
export const Route = createFileRoute("/transform")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "تغيير الصوت، الخلفية، تمويه الوجه، والتأثيرات البصرية — كل شيء في مسار واحد." },
    ],
  }),
...
type FaceRegion = { id: string; x: number; y: number; w: number; h: number; mode: FaceMode; color: string; strength: number };

/* ─── Voice presets ──────────────────────────────── */
const VOICE_PRESETS = [
  { id: "feminine",   label: "أنثوي",     emoji: "🎀", af: "asetrate=44100*1.25,aresample=44100,atempo=0.8" },
  { id: "masculine",  label: "رجولي",     emoji: "🎙️", af: "asetrate=44100*0.82,aresample=44100,atempo=1.22" },
  { id: "robot",      label: "ربوت",      emoji: "🤖", af: "aecho=0.9:0.9:50:0.6,atremolo=10:0.7" },
  { id: "chipmunk",   label: "سنجاب",     emoji: "🐿️", af: "asetrate=44100*1.6,aresample=44100,atempo=0.625" },
...
/* ─── Style presets ─────────────────────────────── */
const STYLE_PRESETS = [
  { id: "cinematic", label: "سينمائي",   vf: "eq=contrast=1.2:saturation=0.85:gamma=1.1,vignette=PI/6" },
  { id: "vintage",   label: "كلاسيكي",   vf: "eq=contrast=0.9:saturation=0.65:brightness=0.04,vignette=PI/4" },
  ...
  { id: "hdr",       label: "HDR",        vf: "eq=contrast=1.35:saturation=1.4:brightness=-0.02,unsharp=5:5:0.9" },
...
function buildArgs(opts: {
  vw: number; vh: number;
  voiceAf: string | null;
  bgEnabled: boolean; bgColor: string; bgSim: number; bgBlend: number;
  bgMode: "solid" | "image"; bgSolid: string; bgHasImg: boolean;
  faceRegions: FaceRegion[];
  styleVf: string | null;
  hasAudio: boolean;
}): { preArgs: string[]; postArgs: string[]; outName: string; needsBgImage: boolean } {
  const outName = "tr_out.mp4";
  const { vw, vh, voiceAf, bgEnabled, bgColor, bgSim, bgBlend, bgMode, bgSolid, bgHasImg, faceRegions, styleVf, hasAudio } = opts;

  const needsComplex = bgEnabled || faceRegions.some(r => r.mode === "blur" || r.mode === "pixelate");
  const needsBgImage  = bgEnabled && bgMode === "image" && bgHasImg;

  const audioOutArgs = hasAudio
    ? (voiceAf ? ["-af", voiceAf, "-c:a", "aac", "-b:a", "128k"] : ["-c:a", "copy"])
    : ["-an"];
  ...
  if (!needsComplex) {
    ...
    if (bgEnabled) {
      vfParts.push(`colorkey=color=${bgColor.replace("#", "0x")}:similarity=${bgSim}:blend=${bgBlend}`);
    }
    for (const r of faceRegions) {
      if (r.mode === "cover")
        vfParts.push(`drawbox=x=${r.x}:y=${r.y}:w=${r.w}:h=${r.h}:color=${r.color.replace("#", "0x")}@1:t=fill`);
    }
    ...
    return {
      ...
      postArgs: [
        ...
        ...audioOutArgs,
        "-c:v", "libx264", "-preset", "ultrafast", "-crf", "18", outName,
      ],
      outName,
      needsBgImage: false,
    };
  ...
  for (const r of faceRegions) {
    const out = `face${n}`;
    if (r.mode === "cover") {
      parts.push(`[${stream}]drawbox=x=${r.x}:y=${r.y}:w=${r.w}:h=${r.h}:color=${r.color.replace("#", "0x")}@1:t=fill[${out}]`);
    } else if (r.mode === "blur") {
      ...
      parts.push(`[${stream}]split=2[${mn}][${sp}]`);
      parts.push(`[${sp}]crop=${r.w}:${r.h}:${r.x}:${r.y},scale=iw/${ps}:-1:flags=neighbor,scale=${r.w}:${r.h}:flags=neighbor[${bl}]`);
      parts.push(`[${mn}][${bl}]overlay=${r.x}:${r.y}[${out}]`);
    }
    stream = out;
    n++;
...
function TransformPage() {
  ...
  const paint = useCallback(() => {
    ...
    else { ctx.fillStyle = "#0f0f1a"; ctx.fillRect(0, 0, cv.width, cv.height); }
    ...
    for (const r of faceRegions) {
      ...
      ctx.strokeStyle = r.id === selectedFaceId ? "#fff" : (r.mode === "blur" ? "#0ea5e9" : r.mode === "pixelate" ? "#8b5cf6" : "#f59e0b");
      ...
      ctx.fillText(r.mode === "blur" ? "تمويه" : r.mode === "pixelate" ? "تكسير" : "تغطية", dx + 4, dy + 14);
    }
  ...
  function onUp() {
    ...
    if (r.w > 6 && r.h > 6) {
      const region: FaceRegion = {
        id: uid(), x: r.x, y: r.y, w: r.w, h: r.h,
        mode: defaultFaceMode, strength: defaultFaceStr, color: defaultFaceColor,
      };
  ...
  async function run() {
    ...
    if (!hasAnyEffect) { toast_("فعّل تأثيراً واحداً على الأقل", "err"); return; }
    if (!videoMeta) return;
    setProcessing(true);
    setProgress(0);
    setFfLog("");
    ...
    try {
      ...
      const { preArgs, postArgs, outName, needsBgImage } = buildArgs({
        vw: videoMeta.w, vh: videoMeta.h,
        voiceAf,
        bgEnabled, bgColor, bgSim, bgBlend, bgMode,
        bgSolid, bgHasImg: !!bgImageFile,
        faceRegions,
        styleVf, hasAudio,
      });
  ...
  const sectionHeader = (label: string, icon: React.ReactNode, open: boolean, setOpen: (v: boolean) => void, enabled: boolean, onToggle: (() => void) | null) => (
  ...
  return (
    <div className="min-h-screen bg-background text-foreground" dir="rtl">
      ...
      {toast && (
        <div className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 rounded-2xl px-5 py-3 text-sm font-medium shadow-2xl backdrop-blur-md border animate-in slide-in-from-top-2 ${toast.type === "ok" ? "bg-emerald-500/20 border-emerald-500/40 text-emerald-300" : "bg-red-500/20 border-red-500/40 text-red-300"}`}>
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
          ...
          <section className="space-y-4">
            {!file ? (
              ...
              >
                <div className="rounded-2xl bg-fuchsia-500/10 p-5"><Upload className="size-10 text-fuchsia-400" /></div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between px-1">
                  <span className="text-xs text-muted-foreground">{file.name} · {videoMeta.w}×{videoMeta.h}px</span>
                  <button onClick={() => { setFile(null); setFaceRegions([]); setVideoMeta(null); frameRef.current = null; if (outputUrl) { URL.revokeObjectURL(outputUrl); setOutputUrl(null); } }}
            ...
            <input ref={bgImgRef} type="file" accept="image/*" className="hidden"
              onChange={(e) => { const f = e.target.files?.[0]; if (f) setBgImageFile(f); e.target.value = ""; }} />
            ...
            {processing && (
              <div className="rounded-xl border border-fuchsia-500/30 bg-fuchsia-500/8 px-4 py-3 space-y-2">
                ...
                <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-fuchsia-500 to-violet-400 rounded-full transition-all duration-300"
                    style={{ width: `${Math.max(4, progress)}%` }} />
                </div>
                {ffLog && <p className="text-[10px] text-muted-foreground/60 font-mono line-clamp-2 break-all">{ffLog}</p>}
              </div>
            )}
            ...
            {hasAnyEffect && (
              <div className="flex flex-wrap gap-2 px-1">
                {voiceEnabled && <Chip color="violet" label={`🎙️ ${VOICE_PRESETS.find(p => p.id === voicePreset)?.label ?? ""}`} />}
                {bgEnabled && <Chip color="sky" label="🖼️ تغيير الخلفية" />}
                {faceRegions.length > 0 && <Chip color="pink" label={`👤 ${faceRegions.length} منطقة وجه`} />}
                {styleEnabled && <Chip color="amber" label={`🎨 ${STYLE_PRESETS.find(p => p.id === stylePreset)?.label ?? ""}`} />}
              </div>
            )}
          </section>
          ...
          <aside className="space-y-3">
            ...
            <div className="rounded-2xl border border-border bg-card/60 overflow-hidden">
              {sectionHeader("تغيير الصوت", <Mic className="size-4" />, voiceOpen, setVoiceOpen, voiceEnabled, () => setVoiceEnabled(e => !e))}
              {voiceOpen && (
                <div className="border-t border-border/50 p-4 space-y-3">
                  <div className="grid grid-cols-4 gap-1.5">
                    {VOICE_PRESETS.map(p => (
                      <button key={p.id} onClick={() => { setVoicePreset(p.id); setVoiceEnabled(true); }}
            ...
            {/* ② Background */}
            <div className="rounded-2xl border border-border bg-card/60 overflow-hidden">
              {sectionHeader("تغيير الخلفية", <ImageIcon className="size-4" />, bgOpen, setBgOpen, bgEnabled, () => setBgEnabled(e => !e))}
              {bgOpen && (
                <div className="border-t border-border/50 p-4 space-y-3">
                  <div className="rounded-lg bg-amber-500/10 border border-amber-500/30 p-2.5">
                    <p className="text-[10px] text-amber-300 leading-relaxed">
                      <strong>يتطلب خلفية ذات لون موحّد</strong> (خضراء، زرقاء، إلخ). فعّل الأداة ثم اختر لون الخلفية الحالية وستُستبدل تلقائياً.
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground mb-1.5">لون الخلفية الحالية (Chroma Key)</p>
                      ...
                      <div className="flex gap-1.5">
                        {["#00ff00", "#0000ff", "#ff0000", "#ffffff"].map(c => (
                          <button key={c} onClick={() => setBgColor(c)}
                            className="size-6 rounded border border-border/40 transition hover:scale-110"
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="range" min={0.05} max={0.6} step={0.01} value={bgSim}
                      onChange={e => setBgSim(+e.target.value)} className="w-full accent-sky-500" />
                  </div>
                  <div className="space-y-1.5">
                    <div className="flex justify-between text-xs">
                    ...
                    <input type="range" min={0} max={0.3} step={0.01} value={bgBlend}
                      onChange={e => setBgBlend(+e.target.value)} className="w-full accent-sky-500" />
                  </div>
                  <div>
                        ...
                        <div className="flex gap-1.5">
                          {["#1e293b", "#ffffff", "#000000", "#0ea5e9"].map(c => (
                            <button key={c} onClick={() => setBgSolid(c)}
                              className="size-6 rounded border border-border/40 hover:scale-110 transition"
                              style={{ background: c }} />
                          ))}
                        </div>
                      </div>
            ...
            {/* ③ Face / Region privacy */}
            <div className="rounded-2xl border border-border bg-card/60 overflow-hidden">
              {sectionHeader("تمويه الوجه والعناصر", <User className="size-4" />, faceOpen, setFaceOpen, faceRegions.length > 0, null)}
              {faceOpen && (
                <div className="border-t border-border/50 p-4 space-y-3">
                  <div className="rounded-lg bg-sky-500/8 border border-sky-500/25 px-3 py-2">
                    <p className="text-[10px] text-sky-300">
                      <Info className="size-3 inline ml-1" />
                      ارسم مستطيلاً على الفيديو فوق أي عنصر (وجه، نص، ملابس، لوحة، إلخ). يمكن رسم مناطق متعددة.
                    </p>
                  </div>
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-1.5">
                      {(["blur", "pixelate", "cover"] as FaceMode[]).map(m => {
                        ...
                        const colors = { blur: "bg-sky-600", pixelate: "bg-violet-600", cover: "bg-amber-600" };
                        return (
                          <button key={m} onClick={() => setDefaultFaceMode(m)}
                  ...
                  {defaultFaceMode !== "cover" && (
                    <div className="space-y-1">
                      <div className="flex justify-between text-xs">
                        <span className="text-muted-foreground">{defaultFaceMode === "blur" ? "شدة التمويه" : "حجم البكسل"}</span>
                        <span className="font-mono">{defaultFaceStr}</span>
                      </div>
                      <input type="range" min={5} max={40} step={1} value={defaultFaceStr}
                  ...
                  {defaultFaceMode === "cover" && (
                    <div className="flex items-center gap-3">
                      ...
                      <input type="color" value={defaultFaceColor} onChange={e => setDefaultFaceColor(e.target.value)}
                        className="size-8 rounded cursor-pointer border border-border bg-transparent" />
                      <div className="flex gap-1.5">
                        {["#000000", "#ffffff", "#1e293b", "#dc2626"].map(c => (
                          <button key={c} onClick={() => setDefaultFaceColor(c)}
                            className="size-5 rounded border border-border/40 hover:scale-110 transition"
                            style={{ background: c }} />
                        ))}
                      </div>
                    </div>
                  ...
                  {faceRegions.length > 0 && (
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-semibold">{faceRegions.length} منطقة مرسومة</span>
                        <button onClick={() => { setFaceRegions([]); setSelectedFaceId(null); }}
                          className="text-xs text-muted-foreground hover:text-destructive flex items-center gap-1 transition">
                      ...
                      <div className="space-y-1 max-h-44 overflow-y-auto">
                        {faceRegions.map((r, i) => (
                          <div key={r.id}
                            ...
                            <div className="size-2.5 rounded-sm shrink-0" style={{ background: r.mode === "blur" ? "#0ea5e9" : r.mode === "pixelate" ? "#8b5cf6" : "#f59e0b" }} />
                            <span className="flex-1">منطقة {i + 1} · {r.w}×{r.h}px</span>
                            <button onClick={(e) => { e.stopPropagation(); setFaceRegions(p => p.filter(x => x.id !== r.id)); if (selectedFaceId === r.id) setSelectedFaceId(null); }}
            ...
            {/* ④ Style */}
            <div className="rounded-2xl border border-border bg-card/60 overflow-hidden">
              {sectionHeader("التأثيرات البصرية والأسلوب", <Palette className="size-4" />, styleOpen, setStyleOpen, styleEnabled, () => setStyleEnabled(e => !e))}
              {styleOpen && (
                <div className="border-t border-border/50 p-4 space-y-3">
                  <div className="grid grid-cols-2 gap-1.5">
                    {STYLE_PRESETS.map(p => (
                      <button key={p.id} onClick={() => { setStylePreset(p.id); setStyleEnabled(true); }}
            ...
            {file && !hasAnyEffect && <p className="text-center text-xs text-muted-foreground">فعّل تأثيراً واحداً على الأقل</p>}
          </aside>
        </div>
      </main>
    </div>
...
function Chip({ color, label }: { color: string; label: string }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-semibold ${cls[color] ?? cls.violet}`}>
      {label}
    </span>
  );
}
