...
function EnhancePage() {
  ...
  const [smartLimits, setSmartLimits] = useState(DEFAULT_SMART_LIMITS);
  const [smartAdvancedOpen, setSmartAdvancedOpen] = useState(false);
  ...
  //  Smart Mode Decision
  // --------------------------------------------------------
  function decideMode(): "local" | "cloud" {
    if (processingMode !== "auto") return processingMode;
    if (!meta) return "local";
    const sizeMB = file ? file.size / (1024 * 1024) : 0;
    if (sizeMB > smartLimits.maxLocalMB) return "cloud";
    if (meta.height > smartLimits.maxLocalResolution) return "cloud";
    if (meta.duration > smartLimits.maxLocalDuration) return "cloud";
    return "local";
  }
  ...
  async function onProcess() {
    ...
    // Smart Mode: route to cloud automatically
    if (decideMode() === "cloud") {
      await enhanceCloud();
      return;
    }
    ...
    try {
      ...
      if (mode === "enhance") {
        ...
        if (denoiseFilter === "nlmeans") vf.push("hqdn3d=6:5:8:6");
        args = ["-i", inputName, "-vf", vf.join(","), ...fastEncodeArgs({ hasAudio, outName })];
        ...
        const timeFilter = textAlwaysShow
          ? ""
          : `:enable='between(t,${textStartTime},${textEndTime})'`;
        ...
        args = ["-i", inputName, "-vf", drawFilter, ...fastEncodeArgs({ hasAudio, outName })];
      } else if (mode === "logo-overlay") {
        if (!logoFile) {
          showToast("الرجاء رفع صورة الشعار أولاً", "err");
          return;
        }
        ...
        if (extraFiles.length === 0) {
          showToast("أضف ملفاً واحداً على الأقل للدمج", "err");
          return;
        }
        await Promise.all(
          extraFiles.map(async (ef, i) => {
        ...
        outName = "thumb.jpg";
        args = ["-ss", String(thumbAt), "-i", inputName, "-frames:v", "1", "-q:v", "2", outName];
        ...
        args = [
          ...
          ...fastEncodeArgs({
            ...
            hasAudio,
            audioArgs: audioArgs.length ? audioArgs : undefined,
            outName,
            tune: "film",
          }),
        ];
  ...
  // 🔥 المعالجة عبر السيرفر /api/enhance
  async function enhanceCloud() {
    if (!file) {
      showToast("ارفع فيديو أولاً", "err");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground" dir="rtl">
      ...
      {toast && (
        <div
          className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 rounded-2xl px-5 py-3 text-sm font-medium shadow-2xl backdrop-blur-md border transition-all animate-in slide-in-from-top-2 ${
            toast.type === "ok"
      ...
      <header className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3.5 border-b border-border/60 sticky top-0 z-40 bg-background/95 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-3">
          <Link
            to="/transform"
            className="text-sm text-muted-foreground hover:text-fuchsia-400 transition"
          >
            تحويل
          </Link>
          <Link
            to="/batch"
            className="text-sm text-muted-foreground hover:text-violet-400 transition"
          >
            دفعي
          </Link>
          <Link
      ...
      <main className="mx-auto max-w-7xl px-4 py-5 grid gap-5 lg:grid-cols-[1fr_360px] xl:grid-cols-[1fr_400px]">
        ...
        <section className="space-y-4 min-w-0">
          ...
          <div
            ...
            } bg-card/50 overflow-hidden aspect-video flex items-center justify-center min-h-60 transition-colors relative`}
            onDragOver={(e) => {
              e.preventDefault();
              setDragOver(true);
            }}
          ...
          >
            {outputUrl ? (
              ...
              <button
                ...
                className="flex flex-col items-center gap-4 text-muted-foreground hover:text-violet-400 transition p-10 w-full h-full group"
              >
                <div className="rounded-3xl border-2 border-dashed border-current p-8 group-hover:border-violet-400 transition">
                  ...
                  <span className="block text-xs mt-1.5 opacity-60">
                    MP4, MOV, AVI, MKV, WebM — حتى 500MB
                  </span>
                </div>
              </button>
            )}
          ...
          {busy && (
            <div className="rounded-xl border border-border bg-card p-4 space-y-2">
              ...
              <div className="h-2.5 bg-muted rounded-full overflow-hidden">
                <progress
                  value={loadingFFmpeg ? 8 : Math.max(4, progress)}
                  max={100}
                  className="h-full w-full appearance-none rounded-full bg-muted"
                  title={loadingFFmpeg ? "تحميل FFmpeg" : `${progress}%`}
                >
                  <span className="sr-only">{loadingFFmpeg ? "تحميل FFmpeg" : `${progress}%`}</span>
                </progress>
              </div>
              {loadingFFmpeg && (
                <p className="text-xs text-muted-foreground">
                  يتم تحميل FFmpeg.wasm (~32MB) مرة واحدة فقط
                </p>
              )}
            </div>
          )}
          ...
          {log && (
            <div className="rounded-xl border border-border bg-card/60">
              ...
              {showLog && (
                <pre className="px-4 pb-4 text-xs whitespace-pre-wrap text-muted-foreground max-h-52 overflow-auto font-mono leading-relaxed border-t border-border/50 pt-3">
                  {log}
                </pre>
              )}
            </div>
          ...
          <button
            onClick={onProcess}
            disabled={!file || busy}
            className="lg:hidden w-full inline-flex items-center justify-center gap-2 rounded-xl bg-violet-600 px-6 py-3.5 font-bold text-white hover:opacity-90 transition disabled:opacity-40 shadow-lg shadow-violet-500/20 text-sm"
          >
        ...
        <aside className="space-y-4">
          ...
          <div className="rounded-2xl border border-border bg-card/60 p-4 space-y-3">
            <h2 className="text-sm font-bold flex items-center gap-2">
              <Settings2 className="size-4 text-muted-foreground" /> طريقة المعالجة
            </h2>
            <div className="flex gap-2">
              <button
                onClick={() => setProcessingMode("auto")}
                className={`flex-1 flex items-center justify-center gap-1.5 rounded-xl px-3 py-2 text-xs font-semibold transition ${
                  processingMode === "auto"
            ...
            {/* Decision preview */}
            {meta && processingMode === "auto" && (
              <p className="text-xs text-muted-foreground flex items-center gap-1.5">
                القرار الحالي:
                <span
                  className={`font-bold ${decideMode() === "local" ? "text-green-400" : "text-blue-400"}`}
                >
                  {decideMode() === "local" ? "⚡ معالجة محلية" : "☁️ معالجة سحابية"}
                </span>
              </p>
            )}
            ...
            {/* Advanced thresholds */}
            <button
              onClick={() => setSmartAdvancedOpen(!smartAdvancedOpen)}
              className="text-xs text-primary underline flex items-center gap-1"
            >
              <ChevronRight
                className={`size-3.5 transition-transform ${smartAdvancedOpen ? "rotate-90" : ""}`}
              />
              {smartAdvancedOpen ? "إخفاء حدود الوضع الذكي" : "ضبط حدود الوضع الذكي"}
            </button>

            {smartAdvancedOpen && (
              <div className="space-y-2 border-t border-border/50 pt-3">
                <NumInput
                  label="أقصى حجم للمعالجة المحلية (MB)"
                  value={smartLimits.maxLocalMB}
                  onChange={(v) => setSmartLimits({ ...smartLimits, maxLocalMB: v })}
                />
                <NumInput
                  label="أقصى دقة للمعالجة المحلية (px)"
                  value={smartLimits.maxLocalResolution}
                  onChange={(v) => setSmartLimits({ ...smartLimits, maxLocalResolution: v })}
                />
                <NumInput
                  label="أقصى مدة للمعالجة المحلية (ثانية)"
                  value={smartLimits.maxLocalDuration}
                  onChange={(v) => setSmartLimits({ ...smartLimits, maxLocalDuration: v })}
                />
              </div>
            )}
          </div>
          ...
          <div className="rounded-2xl border border-border bg-card/60 p-3 space-y-3">
            ...
            {groups.map((group) => (
              <div key={group}>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium px-1 mb-1.5">
                  {group}
                </p>
                <div className="grid grid-cols-2 gap-1.5">
                  {MODES.filter((m) => m.group === group).map((m) => (
                    <button
                      ...
                      className={`flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-medium transition text-right ${
                        mode === m.value
                          ? "bg-violet-600 text-white shadow-sm"
                          : "bg-background/60 border border-border hover:bg-secondary text-muted-foreground hover:text-foreground"
                      }`}
                    >
          ...
          <ModeSettings
            ...
            setTextStartTime={setTextStartTime}
            textEndTime={textEndTime}
            setTextEndTime={setTextEndTime}
            textAlwaysShow={textAlwaysShow}
            setTextAlwaysShow={setTextAlwaysShow}
            logoFile={logoFile}
            logoRef={logoRef}
            ...
            setCropY={setCropY}
            rotateDir={rotateDir}
            setRotateDir={setRotateDir}
            crf={crf}
            setCrf={setCrf}
            upscaleRes={upscaleRes}
            setUpscaleRes={setUpscaleRes}
            targetFps={targetFps}
          ...
          >
            ...
            {busy
              ? loadingFFmpeg
                ? "جاري تحميل FFmpeg..."
                : `معالجة... ${progress}%`
              : `${decideMode() === "cloud" ? "☁️" : "⚡"} تطبيق: ${MODES.find((m) => m.value === mode)?.label}`}
          </button>

          {/* Cloud Enhance Button */}
          <button
            onClick={enhanceCloud}
            disabled={!file || busy}
            className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-6 py-3.5 font-bold text-white hover:opacity-90 transition disabled:opacity-40 shadow-lg shadow-emerald-500/20 text-sm"
          >
            {busy ? <Loader2 className="size-5 animate-spin" /> : <Sparkles className="size-5" />}
            {busy ? "جاري المعالجة عبر السيرفر..." : "تحسين عبر السحابة"}
          </button>
        </aside>
      </main>
...
type ModeSettingsProps = {
  ...
  setAutoLevel: (v: "light" | "balanced" | "strong") => void;
  brightness: number;
  ...
  setDenoiseFilter: (v: "none" | "hqdn3d" | "nlmeans") => void;
  denoiseStrength: "light" | "medium" | "strong";
  setDenoiseStrength: (v: "light" | "medium" | "strong") => void;
  colorPreset: string;
  setColorPreset: (v: string) => void;
  brightness2: number;
  setBrightness2: (v: number) => void;
  contrast2: number;
  setContrast2: (v: number) => void;
  saturation2: number;
  setSaturation2: (v: number) => void;
  gamma2: number;
  setGamma2: (v: number) => void;
  textContent: string;
  ...
  cropW: number;
  setCropW: (v: number) => void;
  cropH: number;
  setCropH: (v: number) => void;
  cropX: number;
  setCropX: (v: number) => void;
  cropY: number;
  setCropY: (v: number) => void;
  rotateDir: "90cw" | "90ccw" | "180" | "fliph" | "flipv";
  setRotateDir: (v: "90cw" | "90ccw" | "180" | "fliph" | "flipv") => void;
  crf: number;
  setCrf: (v: number) => void;
  upscaleRes: "1920x1080" | "1280x720" | "3840x2160";
  setUpscaleRes: (v: "1920x1080" | "1280x720" | "3840x2160") => void;
  targetFps: number;
  ...
  thumbAt: number;
  setThumbAt: (v: number) => void;
  extraFiles: File[];
  setExtraFiles: React.Dispatch<React.SetStateAction<File[]>>;
  extraRef: React.RefObject<HTMLInputElement | null>;
};

function ModeSettings(p: ModeSettingsProps) {
    ...
    return (
      <Card>
        ...
        <Slider
          ...
          max={1}
          step={0.05}
          onChange={p.setBrightness}
          display={p.brightness.toFixed(2)}
        />
        <Slider
          label="التباين"
          value={p.contrast}
          ...
          max={2}
          step={0.05}
          onChange={p.setContrast}
          display={p.contrast.toFixed(2)}
        />
        <Slider
          label="التشبع"
          value={p.saturation}
          ...
          max={3}
          step={0.1}
          onChange={p.setSaturation}
          display={p.saturation.toFixed(1)}
        />
        <Slider
          label="الحدّة"
          value={p.sharpness}
          ...
          max={2}
          step={0.1}
          onChange={p.setSharpness}
          display={p.sharpness.toFixed(1)}
        />
        <Slider
          label="جاما"
          value={p.gamma}
        ...
        <label className="block">
          ...
          <select
            value={p.denoiseFilter}
            onChange={(e) => p.setDenoiseFilter(e.target.value as "none" | "hqdn3d" | "nlmeans")}
        ...
        <div className="grid grid-cols-5 gap-1.5">
          {COLOR_PRESETS.map((cp) => (
            <button
              ...
              className={`rounded-lg py-1.5 text-[10px] font-medium transition ${
                p.colorPreset === cp.id
        ...
        {p.colorPreset === "none" && (
          <>
            <Slider
              ...
              max={1}
              step={0.05}
              onChange={p.setBrightness2}
              display={p.brightness2.toFixed(2)}
              color="pink"
            />
            <Slider
              label="التباين"
              ...
              max={2}
              step={0.05}
              onChange={p.setContrast2}
              display={p.contrast2.toFixed(2)}
              color="pink"
            />
            <Slider
              label="التشبع"
              ...
              max={3}
              step={0.1}
              onChange={p.setSaturation2}
              display={p.saturation2.toFixed(1)}
              color="pink"
            />
            <Slider
              label="جاما"
              ...
              max={2}
              step={0.05}
              onChange={p.setGamma2}
              display={p.gamma2.toFixed(2)}
              color="pink"
            />
          </>
        )}
    ...
    return (
      <Card>
        ...
        <div className="grid grid-cols-2 gap-2">
          <label className="block">
            ...
            <input
              type="number"
              value={p.textSize}
              onChange={(e) => p.setTextSize(+e.target.value)}
              min={16}
              max={200}
          ...
          <label className="block">
            ...
            <select
              value={p.textColor}
              onChange={(e) => p.setTextColor(e.target.value)}
              className="w-full mt-1 rounded-lg bg-input border border-border px-3 py-2 text-xs"
            >
              {["white", "yellow", "red", "lime", "cyan", "orange", "black"].map((c) => (
        ...
        <label className="block">
          ...
          <select
            value={p.textPosition}
            onChange={(e) => p.setTextPosition(e.target.value)}
            className="w-full mt-1 rounded-lg bg-input border border-border px-3 py-2 text-xs"
          >
        ...
        <div className="flex gap-3">
          <label className="flex items-center gap-2 text-xs cursor-pointer">
            <input
              type="checkbox"
              checked={p.textBg}
              onChange={(e) => p.setTextBg(e.target.checked)}
          ...
          <label className="flex items-center gap-2 text-xs cursor-pointer">
            <input
              type="checkbox"
              checked={p.textAlwaysShow}
              onChange={(e) => p.setTextAlwaysShow(e.target.checked)}
              className="accent-sky-500"
            />
            دائماً
          </label>
  ...
  if (p.mode === "logo-overlay")
    return (
      <Card>
        ...
        <div className="rounded-xl border border-dashed border-border p-3 text-center">
          {p.logoFile ? (
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => p.logoRef.current?.click()} className="text-xs text-primary">
                تغيير
              </button>
            </div>
          ) : (
            <button
              ...
              className="flex flex-col items-center gap-1.5 w-full text-muted-foreground hover:text-emerald-400 transition"
            >
          ...
          <div className="grid grid-cols-2 gap-1.5 mt-1.5">
            {[
              ...
              <button
                ...
                onClick={() =>
                  p.setLogoPosition(
                    v as "top-right" | "top-left" | "bottom-right" | "bottom-left" | "center",
                  )
                }
        ...
        <Slider
          label={`حجم الشعار: ${p.logoScale}%`}
          value={p.logoScale}
          min={5}
          max={50}
          step={1}
          onChange={p.setLogoScale}
          display={p.logoScale + "%"}
          color="emerald"
        />
        <Slider
          label={`الشفافية: ${Math.round(p.logoOpacity * 100)}%`}
          value={p.logoOpacity}
          min={0.1}
          max={1}
          step={0.05}
          onChange={p.setLogoOpacity}
          display={Math.round(p.logoOpacity * 100) + "%"}
          color="emerald"
        />
      </Card>
    );
      ...
      <Card>
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {[
            ...
            <button
              key={v}
              onClick={() => p.setDenoiseStrength(v as "light" | "medium" | "strong")}
        ...
        <p className="text-[11px] text-muted-foreground">
          {p.denoiseStrength === "light"
            ? "hqdn3d — سريع جداً"
            : p.denoiseStrength === "medium"
              ? "hqdn3d متوسط — توازن ممتاز"
              : "nlmeans — أعلى جودة، أبطأ"}
        </p>
      </Card>
    );
  ...
  if (p.mode === "speed")
    return (
      <Card>
        ...
        <Slider
          label={`السرعة: ${p.speed.toFixed(2)}x`}
          value={p.speed}
          min={0.25}
          max={4}
          step={0.05}
          onChange={p.setSpeed}
          display={p.speed.toFixed(2) + "x"}
          color="green"
        />
        ...
        <p className="text-[11px] text-muted-foreground">
          ملاحظة: الصوت يُعالج حتى 2x فقط. أسرع من ذلك يُحذف الصوت.
        </p>
      </Card>
    );
      ...
      <Card>
        ...
        {p.file && p.meta?.duration ? (
          ...
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted-foreground">
              المدة: {Math.max(0, p.trimEnd - p.trimStart).toFixed(1)} ثانية
            </p>
          </div>
        )}
      </Card>
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {[
            ...
            <button
              key={v}
              onClick={() => p.setRotateDir(v as "90cw" | "90ccw" | "180" | "fliph" | "flipv")}
              className={`rounded-lg py-2 text-xs font-medium transition ${
                p.rotateDir === v
  ...
  if (p.mode === "compress")
    return (
      <Card>
        ...
        <Slider
          label={`CRF: ${p.crf} (أقل = جودة أعلى)`}
          value={p.crf}
          min={15}
          max={35}
          step={1}
          onChange={p.setCrf}
          display={String(p.crf)}
          color="orange"
        />
    ...
    return (
      <Card>
        ...
        {[
          ...
          <button
            key={v}
            onClick={() => p.setUpscaleRes(v as "1920x1080" | "1280x720" | "3840x2160")}
  ...
  if (p.mode === "concat")
    return (
      <Card>
        ...
        <p className="text-[11px] text-muted-foreground">
          الفيديو الرئيسي + الملفات التالية ستُدمج بالترتيب.
        </p>
        <button
          onClick={() => p.extraRef.current?.click()}
        ...
        {p.extraFiles.length > 0 && (
          <div className="space-y-1.5 max-h-32 overflow-auto">
            {p.extraFiles.map((f, i) => (
              <div
                key={i}
                className="flex items-center justify-between text-xs bg-background/60 border border-border rounded-lg px-3 py-1.5"
              >
                <span className="truncate">
                  {i + 1}. {f.name}
                </span>
                <button
                  onClick={() => p.setExtraFiles((prev) => prev.filter((_, idx) => idx !== i))}
        ...
        <p className="text-[11px] text-muted-foreground">
          تأكد أن جميع الفيديوهات بنفس الدقة والصيغة للحصول على أفضل نتيجة.
        </p>
      </Card>
    );

  if (p.mode === "extract-audio")
    return (
      <Card>
  ...
  if (p.mode === "reverse")
    return (
      <Card>
        ...
        <p className="text-xs text-muted-foreground">
          يعكس الفيديو والصوت معاً. تنبيه: ملفات كبيرة تستغرق وقتاً.
        </p>
      </Card>
    );
  ...
  if (p.mode === "stabilize")
    return (
      <Card>
        ...
        <p className="text-xs text-muted-foreground">
          يتطلب مرحلتين: تحليل → تثبيت. قد يستغرق وقتاً للملفات الطويلة.
        </p>
      </Card>
    );
  ...
  if (p.mode === "auto-enhance") {
    ...
    const ops: { label: string; detail: string; active: boolean }[] = [
      {
        label: "إزالة التشويش",
        detail: p.autoLevel === "strong" ? "hqdn3d قوي" : "hqdn3d متوسط",
        active: true,
      },
      {
        label: "تحسين الألوان",
      ...
      {
        label: "ترقية الدقة",
        detail: p.autoLevel === "balanced" ? "→ 720p" : "→ 1080p",
        active: willUpscale,
      },
      {
        label: "تطبيع الصوت",
        detail: "loudnorm EBU R128",
        active: p.autoLevel !== "light",
      },
    ];
    return (
      <Card>
            ...
            <div
              ...
              className={`flex items-center justify-between rounded-lg px-3 py-2 text-xs transition ${
                op.active
                  ...
                  : "bg-background/40 border border-border opacity-40"
              }`}
            >
              <div className="flex items-center gap-2">
                <div
                  className={`size-4 rounded-full flex items-center justify-center ${
                    op.active ? "bg-amber-500" : "bg-muted"
                  }`}
                >
                  {op.active && <Check className="size-2.5 text-white" strokeWidth={3} />}
                </div>
                <span
                  className={op.active ? "text-foreground font-medium" : "text-muted-foreground"}
                >
                  {op.label}
                </span>
              </div>
        ...
        {!p.meta && (
          <p className="text-[11px] text-amber-400/80 mt-1">ارفع فيديو لرؤية التحسينات المقترحة</p>
        )}
      </Card>
    );
  }
...
function WaveformTrimmer({
      ...
      .then((decoded) => {
        ...
        for (let i = 0; i < BAR_COUNT; i++) {
          ...
          for (let j = 0; j < blockSize; j++)
            mx = Math.max(mx, Math.abs(ch[i * blockSize + j] ?? 0));
          out[i] = mx;
        }
        setBars(out);
      })
  ...
  useEffect(() => {
    ...
    if (!el) return;
    el.style.setProperty("--trim-start", `${startPct.toFixed(4)}%`);
    el.style.setProperty("--trim-end", `${endPct.toFixed(4)}%`);
  }, [startPct, endPct]);
  ...
  return (
    <div className="space-y-2.5">
      <div
        ref={containerRef}
        className="waveform-trimmer relative h-16 rounded-xl overflow-hidden bg-black/30 border border-border cursor-col-resize select-none"
      ...
      >
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div className="waveform-overlay-left absolute inset-y-0 left-0 bg-black/55" />
          <div className="waveform-overlay-right absolute inset-y-0 right-0 bg-black/55" />
          <div className="waveform-selection absolute inset-y-0 border-2 border-violet-400/70 rounded" />
        </div>

        {/* Start handle */}
        <div
          className="waveform-handle-start absolute inset-y-0 w-3 bg-violet-500 hover:bg-violet-400 cursor-ew-resize flex items-center justify-center touch-none z-10"
          onPointerDown={(e) => {
            e.preventDefault();
        ...
        {/* End handle */}
        <div
          className="waveform-handle-end absolute inset-y-0 w-3 bg-violet-500 hover:bg-violet-400 cursor-ew-resize flex items-center justify-center touch-none z-10"
          onPointerDown={(e) => {
            e.preventDefault();
            e.currentTarget.setPointerCapture(e.pointerId);
            setDragging("end");
      ...
      <div className="grid grid-cols-2 gap-2">
        <NumInput
          label="من (ثانية)"
          value={trimStart}
          onChange={(v) => setTrimStart(Math.max(0, Math.min(v, trimEnd - 0.1)))}
        />
        <NumInput
          label="إلى (ثانية)"
          value={trimEnd}
          onChange={(v) => setTrimEnd(Math.min(duration, Math.max(v, trimStart + 0.1)))}
        />
      </div>

      <p className="text-[11px] text-muted-foreground">
        المحدد:{" "}
        <span className="text-violet-400 font-semibold">{fmtDuration(trimEnd - trimStart)}</span> من
        أصل {fmtDuration(duration)} · اسحب المقابض البنفسجية
      </p>
    </div>
  );
}
