...
export const Route = createFileRoute("/fingerprint")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "إعادة تشفير الفيديو وتغيير البصمة الرقمية وجميع البيانات الوصفية.",
      },
    ],
  }),
        ...
        <section className="space-y-4">
          <div className="rounded-2xl border border-border bg-card/60 overflow-hidden aspect-video flex items-center justify-center min-h-[260px]">
            {outputUrl ? (
              ...
              <button
                ...
                className="flex flex-col items-center gap-4 text-muted-foreground hover:text-amber-400 transition p-12 w-full h-full"
              >
          ...
          {log && (
            <details className="rounded-xl border border-border bg-card/60 p-3" open>
              ...
              <pre className="mt-2 text-xs whitespace-pre-wrap text-muted-foreground max-h-48 overflow-auto font-mono">
                {log}
              </pre>
            </details>
          )}
        ...
        <aside className="space-y-4">
                ...
                [
                  {
                    value: "all",
                    label: "شامل (موصى به)",
                    desc: "إعادة تشفير + إزالة metadata + حقن ضوضاء",
                  },
                  {
                    value: "full-reencode",
                    label: "إعادة تشفير فقط",
                    desc: "تغيير كامل لبنية الملف",
                  },
                  {
                    value: "strip-meta",
                    label: "إزالة Metadata فقط",
                    desc: "سريع جداً — يحذف EXIF والبيانات",
                  },
                  {
                    value: "noise-inject",
                    label: "حقن ضوضاء دقيقة",
                    desc: "يغير hash الملف بدون تغيير مرئي",
                  },
                ] as const
              ).map((m) => (
                <button
          ...
          {(method === "full-reencode" || method === "all") && (
            <div className="rounded-2xl border border-border bg-card/60 p-4 space-y-3">
              ...
              <Slider
                label={`CRF: ${crfVariance}`}
                value={crfVariance}
                min={18}
                max={32}
                step={1}
                onChange={setCrfVariance}
                display={String(crfVariance)}
              />
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  type="checkbox"
                  checked={addSubtle}
                  onChange={(e) => setAddSubtle(e.target.checked)}
              ...
              {addSubtle && (
                <Slider
                  label={`مستوى الضوضاء: ${noiseLevel}`}
                  value={noiseLevel}
                  min={1}
                  max={5}
                  step={1}
                  onChange={setNoiseLevel}
                  display={String(noiseLevel)}
                />
              )}
            </div>
          )}
          ...
          {method === "noise-inject" && (
            <div className="rounded-2xl border border-border bg-card/60 p-4 space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                مستوى 1-2 لا يُلاحظ بالعين. مستوى 3+ قد يكون مرئياً في الضوء القوي.
              </p>
            </div>
          )}
          ...
          <div className="rounded-2xl border border-border bg-card/60 p-4 space-y-3">
            ...
            <TextInput
              ...
              placeholder="أتركه فارغاً للحذف"
            />
            <TextInput
              label="الفنان (artist)"
              value={newArtist}
            ...
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              <input
                type="checkbox"
                checked={changeTimestamp}
                onChange={(e) => setChangeTimestamp(e.target.checked)}
                className="accent-amber-500"
              />
