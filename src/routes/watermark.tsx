...
import {
  ...
  Droplets, Trash2, Plus, Wand2, Square, Eraser, X, Info,
} from "lucide-react";
...
export const Route = createFileRoute("/watermark")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "إزالة الشعارات والنصوص والعلامات المائية من الفيديو — delogo، ضبابية، أو تغطية." },
    ],
  }),
...
const MODE_META: Record<RegionMode, { label: string; desc: string; color: string; border: string }> = {
  delogo: { label: "استرداد",  desc: "يملأ المنطقة بحسب المحيط — الأفضل للشعارات",  color: "#8b5cf6", border: "border-violet-500/40" },
  blur:   { label: "ضبابية",   desc: "يضبب المنطقة — مثالي للنصوص والوجوه",          color: "#0ea5e9", border: "border-sky-500/40" },
  fill:   { label: "تغطية",    desc: "يغطي بلون صلب — سريع وبسيط",                   color: "#f59e0b", border: "border-amber-500/40" },
};
...
function buildFiltergraph(
  ...
  if (!hasBlur) {
    const vf = regions.map(r => {
      if (r.mode === "delogo")
        return `delogo=x=${r.x}:y=${r.y}:w=${r.w}:h=${r.h}:show=0`;
      return `drawbox=x=${r.x}:y=${r.y}:w=${r.w}:h=${r.h}:color=${r.fillColor.replace("#", "0x")}@1:t=fill`;
    }).join(",");
    return {
      args: ["-vf", vf, "-c:v", "libx264", "-preset", "ultrafast", "-crf", "18", ...audioArgs, outName],
      outName,
    };
  }
  ...
  for (const r of regions) {
    ...
    if (r.mode === "delogo") {
      ...
      parts.push(`[${stream}]drawbox=x=${r.x}:y=${r.y}:w=${r.w}:h=${r.h}:color=${r.fillColor.replace("#", "0x")}@1:t=fill[${out}]`);
    } else {
  ...
  function onUp() {
    ...
    if (r.w > 6 && r.h > 6) {
      const region: Region = {
        ...
        mode: defaultMode, blurStrength: defaultBlur, fillColor: defaultFill,
      };
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_340px]">
          ...
          <section className="space-y-4">
            ...
            {showTip && file && (
              <div className="flex items-start gap-3 rounded-xl border border-sky-500/30 bg-sky-500/8 px-4 py-3">
                ...
                <p className="text-xs text-sky-300 flex-1">
                  اختر طريقة الإزالة من القائمة، ثم <strong>اسحب مستطيلاً</strong> فوق الشعار أو النص على الصورة. يمكنك رسم عدة مناطق بطرق مختلفة.
                </p>
            ...
            {!file ? (
              ...
              >
                <div className="rounded-2xl bg-rose-500/10 p-5"><Droplets className="size-10 text-rose-400" /></div>
              ...
              videoMeta && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between px-1">
                    ...
                    <button
                      onClick={() => { setFile(null); setRegions([]); setVideoMeta(null); frameRef.current = null; if (outputUrl) { URL.revokeObjectURL(outputUrl); setOutputUrl(null); } }}
            ...
            {processing && (
              <div className="rounded-xl border border-violet-500/30 bg-violet-500/8 px-4 py-3 space-y-2">
                ...
                {ffLog && <p className="text-[10px] text-muted-foreground/60 font-mono leading-relaxed line-clamp-2 break-all">{ffLog}</p>}
              </div>
            )}
            ...
            <div className="rounded-2xl border border-border bg-card/60 p-4 space-y-3">
                ...
                {(Object.keys(MODE_META) as RegionMode[]).map(m => (
                  <button key={m} onClick={() => setDefaultMode(m)}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className={`text-[10px] mt-0.5 ${defaultMode === m ? "text-white/70" : "text-muted-foreground"}`}>{MODE_META[m].desc}</div>
                    </div>
                  </button>
                ))}
              </div>
              ...
              {defaultMode === "fill" && (
                <div className="flex items-center gap-3 pt-1 border-t border-border/40">
                  ...
                  <input type="color" value={defaultFill} onChange={e => setDefaultFill(e.target.value)}
            ...
            <div className="rounded-2xl border border-border bg-card/60 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-bold flex items-center gap-2">
                  ...
                  المناطق المحددة
                  {regions.length > 0 && (
                    <span className="rounded-full bg-rose-500/20 px-2 py-0.5 text-[10px] font-bold text-rose-400">{regions.length}</span>
                  )}
                </h2>
                {regions.length > 0 && (
                        ...
                        <div className="mt-3 space-y-2 border-t border-border/40 pt-2.5">
                          ...
                          {r.mode === "fill" && (
                            <div className="flex items-center gap-2">
                              ...
                              <input type="color" value={r.fillColor}
                                onClick={e => e.stopPropagation()}
