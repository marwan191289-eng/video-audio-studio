      ...
      <main className="mx-auto max-w-6xl px-6 py-10">
        ...
        {isLoading ? (
          ...
          <div className="rounded-xl border border-border bg-card p-12 text-center">
            ...
            <Link to="/enhance" className="inline-flex rounded-lg bg-primary px-5 py-2.5 font-semibold text-primary-foreground">
              ابدأ معالجة فيديو
            </Link>
          </div>
        ) : (
          <div className="grid gap-3">
            {data.map((row) => (
              <div key={row.id} className="rounded-xl border border-border bg-card p-4 flex items-center justify-between gap-4">
                ...
                <div className="flex gap-2 shrink-0">
                  <button onClick={() => download(row)} className="rounded-lg border border-border bg-card p-2 hover:bg-secondary" title="تنزيل">
