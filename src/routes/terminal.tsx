...
import {
  ArrowRight, Terminal as TerminalIcon, Upload, Download, Trash2, Copy, Search, Play, ChevronDown,
} from "lucide-react";
...
export const Route = createFileRoute("/terminal")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "نفّذ أوامر FFmpeg ومعالجة الفيديو بلغات متعددة مباشرة في المتصفح." },
    ],
  }),
...
function TerminalPage() {
  ...
  function onKey(e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === "Enter" && !busy) { const v = input; setInput(""); run(v); }
      ...
      if (hIdx === null) return;
      const idx = hIdx - 1;
      if (idx < 0) { setHIdx(null); setInput(""); }
  ...
  const filtered = SNIPPETS.filter((s) => {
    ...
    const matchText = !filter || s.title.toLowerCase().includes(filter.toLowerCase()) ||
      s.code.toLowerCase().includes(filter.toLowerCase()) || s.desc.toLowerCase().includes(filter.toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-background text-foreground" dir="rtl">
      ...
      <main className="mx-auto max-w-7xl px-6 py-5 grid gap-5 lg:grid-cols-[1fr_380px]" style={{ height: "calc(100vh - 72px)" }}>
        {/* Terminal */}
        <section className="rounded-2xl border border-border bg-[#080c12] overflow-hidden flex flex-col">
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-border/50 bg-[#0d1117]">
            ...
            <span className="text-xs text-muted-foreground font-mono">ffmpeg@wasm:~ — Video Enhancer Pro</span>
              ...
              {ALL_LANGS.map((l) => (
                <button
                  ...
                  className={`rounded-full px-2.5 py-0.5 text-[10px] font-medium transition ${
                    langFilter === l ? "bg-primary text-primary-foreground" : "bg-background border border-border text-muted-foreground hover:border-primary/50"
                  }`}
                >
                  {l}
                </button>
            ...
            {filtered.map((s) => (
              <div key={s.id} className="rounded-xl border border-border bg-background/40 p-2.5 hover:border-primary/30 transition">
                <div className="flex items-center justify-between gap-2 mb-1.5">
                  ...
                  <span className={`text-[9px] uppercase border rounded px-1.5 py-0.5 shrink-0 font-mono ${
                    s.lang === "ffmpeg" ? "border-sky-500/40 text-sky-400" :
                    s.lang === "python" ? "border-yellow-500/40 text-yellow-400" :
                    s.lang === "javascript" || s.lang === "nodejs" ? "border-amber-500/40 text-amber-400" :
                    s.lang === "bash" ? "border-emerald-500/40 text-emerald-400" :
                    s.lang === "powershell" ? "border-blue-500/40 text-blue-400" :
                    "border-border text-muted-foreground"
                  }`}>{s.lang}</span>
                </div>
                {s.desc && <p className="text-[10px] text-muted-foreground mb-1.5">{s.desc}</p>}
                <pre dir="ltr" className="text-[10px] font-mono whitespace-pre-wrap break-all text-gray-400 bg-[#080c12] rounded-lg p-2 max-h-28 overflow-auto border border-border/50">{s.code}</pre>
